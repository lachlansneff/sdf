...
Intersection sphere_march(vec3 origin, vec3 ray_dir) {
...
//     Intersection marched_ray = sphere_march(ray_origin, ray_dir);
...
void main(void) {
    ...
    Intersection marched_ray = sphere_march(uniforms.eye, ray_dir);
    ...
    // vec3 shade = vec3(125.0/255.0, 105.0/255.0, 74.0/255.0);
    vec3 shade = vec3(99.0/255.0, 84.0/255.0, 59.0/255.0);
    ...
    float dif = dot(normals, normalize(uniforms.light_pos));
    // color += dif;
    color = mix(color, shade, dif);
